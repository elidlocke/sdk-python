"""Tests for the pdb / breakpoint() fix in workflow code (#1104)."""
...
def test_breakpoint_hook_raises_on_workflow_thread(client: Client):
    ...
    # Constructing a Worker installs the hook. We don't need to run anything.
    Worker(
        client,
        workflows=[ThreadCaptureWorkflow],
        task_queue=f"tq-{uuid.uuid4()}",
    )
    assert sys.breakpointhook is _temporal_workflow_breakpoint_hook

    captured: list[BaseException] = []

    def call_breakpoint_on_worker_thread() -> None:
        try:
            sys.breakpointhook()
    ...
    t = threading.Thread(
        target=call_breakpoint_on_worker_thread,
        name="temporal_workflow__test",
    )
    t.start()
    t.join()
    ...
    assert "debug_mode=True" in str(captured[0])
